...
describe('TheopetraAuthority', function () {
  ...
  describe('Whitelist Signer', function () {
    ...
    it('allows the governor to set a new whitelist signer address', async function () {
      ...
      expect(await TheopetraAuthority.pushWlSigner(alice.address, true));
      expect(await TheopetraAuthority.wlSigner()).to.equal(alice.address);
    });
    ...
    it('allows the governor to set a new whitelist signer address that can be pulled by the new signer', async function () {
      ...
      const [, alice] = users;
      expect(await TheopetraAuthority.pushWlSigner(alice.address, false));
      expect(await TheopetraAuthority.wlSigner()).to.equal(owner);
      expect(await alice.TheopetraAuthority.pullWlSigner());
      expect(await TheopetraAuthority.wlSigner()).to.equal(alice.address);
    });
    ...
    it('should revert if a user other than the governor makes a call to set the whitelist signer address', async function () {
      const { users } = await setup();
      const [, alice] = users;
      await expect(alice.TheopetraAuthority.pushWlSigner(alice.address, true)).to.be.revertedWith('UNAUTHORIZED');
    });
